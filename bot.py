...
# أقصى وقت ننتظره للبحث قبل ما نعطي المستخدم رسالة خطأ (بالثواني)
SEARCH_TIMEOUT = 15

# كاش بسيط بالذاكرة عشان الأغاني المتكررة تطلع فوراً بدون بحث من جديد
_song_cache = {}
_SONG_CACHE_MAX = 200
...
async def extract_song(query: str):
    ...
    def _extract():
        ...
        if "entries" in info:
            if not info["entries"]:
                raise ValueError("ما فيه نتائج لهذا البحث")
            info = info["entries"][0]
        return info

    info = await asyncio.wait_for(loop.run_in_executor(None, _extract), timeout=SEARCH_TIMEOUT)
    song = {
...
async def safe_connect_voice(channel: discord.VoiceChannel):
    """
    الدالة الوحيدة المسؤولة عن الاتصال بالروم الصوتي.
    تستخدم قفل (lock) عشان تمنع أكثر من محاولة اتصال بنفس الوقت،
    اللي كانت تسبب خطأ 4006 (تعارض جلسات صوتية).
    """
    guild = channel.guild
    state = get_state(guild.id)
    ...
    async with state["connect_lock"]:
        ...
        try:
            new_vc = await channel.connect(timeout=30, reconnect=False, self_deaf=True)
...
async def on_message(message: discord.Message):
    ...
    if content.startswith("ش"):
        ...
        cache_key = query.strip().lower()
        is_cached = cache_key in _song_cache
        if not is_cached:
            await message.channel.send(f"🔎 يبحث عن: **{query}** ...")
        try:
            song = await extract_song(query)
        except asyncio.TimeoutError:
            await message.channel.send("⚠️ البحث أخذ وقت طويل بدون رد، جرب مرة ثانية أو بكلمة مختلفة.")
            return
        except Exception as e:
        ...
        vc = state["voice_client"]
        if vc is None or not vc.is_connected():
            target_channel = bot.get_channel(VOICE_CHANNEL_ID) if VOICE_CHANNEL_ID else None
            if target_channel is not None and not state["connect_lock"].locked():
                vc = await safe_connect_voice(target_channel)
            if vc is None or not vc.is_connected():
